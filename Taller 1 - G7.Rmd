...
WAGES <- db2 %>% 
  group_by(Occupation, Sexo) %>%  # Now also group by sex (gender)
  summarise(
    mean_age = mean(age, na.rm = TRUE),
    mean_y_total_m = mean(y_salary_m, na.rm = TRUE),
    na_or_zero_percentage_total = sum(is.na(y_salary_m) | y_salary_m <= 10000) / n() * 100
  ) %>%
  ungroup()
WAGES <- WAGES %>% mutate(across(where(is.numeric), ~ round(.x, 0)))

stargazer(WAGES, type = "latex", summary = FALSE, title = "Formal Workers Summary",
          label = "tab:formal_workers", out = "2.1 WAGES.tex",
          digits = 1)
...
lM1 <- lm(y_salary_m ~ impa_oi + formal + maxEducLevel3 + maxEducLevel4 
...
LEV <- ggplot(db2 , aes(y = leverage , x = id , color= Occupation, shape= as.factor(sex) )) +
  geom_point() + 
  theme_bw() + 
  labs(x = "Observations",  
       y = "Leverage",
...
RES <- ggplot(db2 , aes(y = leverage , x = residuals  )) +
...
lM2 <- lm(y_salary_m ~ impa_oi + formal + maxEducLevel3 + maxEducLevel4 + maxEducLevel5 + maxEducLevel6 + maxEducLevel7 , data = db2)
summary(lM2)
db2$y_salary_m_P <- predict(lM1, newdata = db2)
db2$y_salary_m <- ifelse(is.na(db2$y_salary_m), db2$y_salary_m_P,db2$y_salary_m)
...
rm(db_miss,question, M,Nobs,mode_edu,dummy_maxEducLevel,lM1, Occupied_description,imp_vars,obs_vars,titles,Stats,Obs_vs_Imp, imp_idx,obs_idx,cols, check,lM2,lM1,N,WAGES,TOTAL_INCOME,RES,LEV)
```

```{r Data exploration, echo=FALSE, message=FALSE}
...
stargazer(model_salary, type = "latex",
          ...
          out = "Regression_P3.tex",
          notes = c("*Labor monthly income is measured in logarithmic form. It includes income from both formal and informal work, with labor as the main income source, including tips and commissions. The data is sourced from the Gran Encuesta Integrada de Hogares (GEIH), with extreme values imputed.",
                    "Statistical significance levels: *** p<0.01, ** p<0.05, * p<0.1.")
)

stargazer(model_salary_st, type = "latex",
          title = "Monthly Income Salary Relation with Age (Standardized)",
          covariate.labels = c("Age**", "Age Squared"),
          dep.var.labels = "Labor Monthly Income*",
          star.cutoffs = c(0.1, 0.05, 0.01),
          ci = TRUE, ci.level = 0.95,
          out = "Regression_P3_sd.tex",
          notes = c("*Labor monthly income is measured in logarithmic form and standardized. It includes income from both formal and informal work, with labor as the main income source, including tips and commissions. The data is sourced from the Gran Encuesta Integrada de Hogares (GEIH), with extreme values imputed.",
...
##Explore the regularization of the models given the high parametrization that some of them have.


library(glmnet)

# Define response variable
y <- log(data$y_salary_m)

# Models to regularize

formulas <- list(
  formula_1 = log(y_salary_m) ~ age + I(age^2),
  formula_2 = log(y_salary_m) ~ female,
  formula_3 = log(y_salary_m) ~ female + age + I(age^2) + factor(maxEducLevel) + informal + factor(estrato1),
  formula_4 = log(y_salary_m) ~ female + age + I(age^2) + factor(maxEducLevel) + informal + factor(estrato1) + factor(oficio2),
  formula_5 = log(y_salary_m) ~ female * factor(maxEducLevel) + age + I(age^2) + informal + factor(estrato1) + factor(oficio2),
  formula_6 = log(y_salary_m) ~ female * factor(maxEducLevel) + poly(age, 3) + informal * factor(maxEducLevel) + factor(estrato1) + factor(oficio2),
  formula_7 = log(y_salary_m) ~ female * factor(maxEducLevel) + poly(age, 3) + informal * factor(maxEducLevel) + factor(estrato1) * factor(maxEducLevel) + factor(oficio2),
  formula_8 = log(y_salary_m) ~ female * factor(maxEducLevel) * factor(estrato1) + poly(age, 3) + informal * factor(maxEducLevel) + factor(oficio2)
)
...
for (i in seq_along(formulas)) {
  ...
  X <- model.matrix(formulas[[i]], data)[,-1]  # Remove intercept
  
  # Fit LASSO model
  lasso_model <- glmnet(X, y, alpha = 1)
  
  # Extract coefficients at lambda = 0.2
  coefs <- coef(lasso_model, s = 0.2)
  
  # Store results
  results[[names(formulas)[i]]] <- list(
    model = lasso_model,
    coefficients = coefs
  )
  
  # Plot coefficient paths
  plot(lasso_model, xvar = "lambda", main = paste("LASSO for", names(formulas)[i]))
}

# Display results for one model example
results$formula_1$coefficients
