...
p_load(stringr, # html to text
       ...
       stargazer, # Regression and summary tables
       boot, # Bootstrap methods  
       car, # Advanced regression analysis  
       lmtest, # Statistical tests for linear models  
       caret) # Modeling and validation in machine learning  
```
...
db2 <- db2 %>%
  select(y_ingLab_m, y_salary_m, y_total_m, ie, imdi, impa, isa, iof1, iof2, iof3h, iof3i, iof6, 
         ...
         sex, age, oficio, fex_c, depto, fex_dpto, fweight, informal)
...
check <- function(db, imp_var, missing_var) {
  nrow(db %>% filter(!is.na(!!sym(imp_var)), !!sym(missing_var) != 1)) == 0
}
...
#  Choosing total: 
t.test(db_temp$ingtot_oi, db_temp$y_total_m, paired = TRUE)
...
db2 <- db2 %>%
  mutate(y_salary_m = ifelse(is.na(y_salary_m) & p6240 == 1 & formal == 1, y_salary_m_P, y_salary_m))
...
db2 <- db2 %>%
  mutate(y_gananciaIndep_m = ifelse(is.na(y_gananciaIndep_m) & p6240 == 1 & informal == 1, y_gananciaIndep_m_P, y_gananciaIndep_m))
...
#   data_obs <- db[[obs_vars[i]]]
#   data_imp <- db[[imp_vars[i]]]
#   
...
# --> 2 Options.
model_ingLab <- lm(log(y_ingLab_m) ~ age + I(age^2), data = db2)
model_salary <- lm(log(y_salary_m) ~ age + I(age^2), data = db2)

# y_ingLab_m
ggplot(db2, aes(x = age, y = log(y_ingLab_m))) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), color = "blue") +
  ggtitle("Quadratic Regression: log(y_ingLab_m) ~ Age")
