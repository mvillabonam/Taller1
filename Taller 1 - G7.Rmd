...
# ----> PROOF OF: imputed variables convering some missing values of observed income.
...
# As we can see, imputed variables only cover certain NA values.
# ----> Imputing
...
lM1 <- lm(y_salary_m ~ impa_oi + maxEducLevel3 + maxEducLevel4 
...
lM1 <- lm(y_gananciaIndep_m ~ impa_oi + informal + maxEducLevel3 
...
#   db2 %>%
#     group_by(p6240 = case_when(as.character(p6240),
#                                 `6` = "Other activity",
...
rm(db_miss,question, M,Nobs,mode_edu,quantiles,dummy_maxEducLevel,lM1,y_ingLab_m_P, Occupied_description,imp_vars,obs_vars,titles,Stats,Obs_vs_Imp, imp_idx,imp_vars,obs_idx,cols, check)
