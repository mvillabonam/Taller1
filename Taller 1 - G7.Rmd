...
set.seed(1541)
split = initial_split(db2, prop = 0.7, strata = fex_c)
...
```{r}
model_1 = formula(log(y_salary_m) ~ age + I(age^2))
model_2 = formula(log(y_salary_m) ~ female)
model_3 = formula(log(y_salary_m) ~ female + controls)
model_4 = formula(log(y_salary_m) ~ female + age + I(age^2) + 
                    factor(maxEducLevel) + estrato + educ_padres)
model_5 = formula(log(y_salary_m) ~ I(age^2) + female * age
                    factor(maxEducLevel) + estrato + educ_padres)
