...
# Calculate robust standard errors to address certain violations of the classic linear regression assumptions. 
rse_unconditional <- sqrt(diag(vcovHC(model_unconditional, type = "HC1")))
rse_conditional <- sqrt(diag(vcovHC(conditional_model, type = "HC1")))
rse_fwl <- sqrt(diag(vcovHC(fwl_model_1, type = "HC1")))

#Export to latex
stargazer(
  model_unconditional, conditional_model, fwl_model_1, fwl_boot_model,
...
in_train = createDataPartition(y = data$impa_oi, 
                               p = 0.7, 
                               list = FALSE)
training = data %>% 
...
set.seed(1541)
train_parallel <- function() {
  
  model <- train(formula_7, data = data, method = 'lm', trControl = ctrl_par)
  
  return(model)
}

time_par <- system.time(mod_par <- train_parallel())

stopCluster(cl)  # Detener el clúster después de entrenar
registerDoSEQ()  # Volver a ejecución secuencial

print(paste("Tiempo paralelo:", time_par["elapsed"], "segundos"))
...
time_furr <- system.time(rmse_mod7 <- future_map_dbl(1:nrow(data), ~ loocv_model(.x, data, formula_7)))
...
# Control de entrenamiento con paralelización activada
ctrl_par <- trainControl(method = "LOOCV", verboseIter = TRUE, allowParallel = TRUE)

# Control secuencial
ctrl_seq <- trainControl(method = "LOOCV", verboseIter = TRUE, allowParallel = FALSE)

# Medir tiempo secuencial
registerDoSEQ()  # Asegurar que corra en modo secuencial
time_seq <- system.time(mod_seq <- train(formula_7, data = data2, method = 'lm', trControl = ctrl_seq))

# Medir tiempo paralelo
registerDoParallel(cl)  # Asegurar que use los núcleos disponibles
time_par <- system.time(mod_par <- train(formula_7, data = data2, method = 'lm', trControl = ctrl_par))

# Detener cluster
stopCluster(cl)
registerDoSEQ()  # Volver a ejecución normal

# Mostrar tiempos
print(paste("Tiempo secuencial:", time_seq["elapsed"], "segundos"))
print(paste("Tiempo paralelo:", time_par["elapsed"], "segundos"))
