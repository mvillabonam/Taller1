...
```
The coefficient of -0.23897 indicates that, on average, women have approximately 23.9% lower wages than men. However, the model's low explanatory power suggests that gender alone doesn’t explain much of the variation in wages, this implies that other factors not included in this model may play a significant role in determining wages. Let’s see how the conditional wage gap fix it.

```{r Conditional Gender Wage Gap}


# Conditional Gender Wage Gap
conditional_model <- lm(log(y_salary_m) ~ female + age + maxEducLevel + informal+estrato1+p6050, data = db2)
summary(conditional_model)

#Estimation with Frisch-Waugh-Lovell Theorem (FWL)

# Fit the control model for y_salary_m
controls_model_y <- lm(log(y_salary_m) ~ age + maxEducLevel + depto + oficio+p6050, data = db2)
residuals_controls_y <- residuals(controls_model_y)

#Fit the control model for sex
controls_model_sex <- lm(sex ~ age + maxEducLevel + depto + oficio, data = db2)
residuals_controls_sex <- residuals(controls_model_sex)

#FWL regression
fwl_model <- lm(residuals_controls_y ~ residuals_controls_sex)
summary(fwl_model)


#Estimation using FWL with bootstrap

fwl_function <- function(data, indices) {
    resampled_data <- data[indices, ]
    controls_model_y <- lm(log(y_salary_m) ~ age + maxEducLevel + depto + oficio, data = resampled_data)
    residuals_controls_y <- residuals(controls_model_y)

    controls_model_sex <- lm(sex ~ age + maxEducLevel + depto + oficio, data = resampled_data)
    residuals_controls_sex <- residuals(controls_model_sex)
    
    fwl_model <- lm(residuals_controls_y ~ residuals_controls_sex)
    return(coef(fwl_model)["residuals_controls_sex"])
}
bootstrap_results <- boot(data = db2, statistic = fwl_function, R = 1000)

# Display bootstrap results
bootstrap_estimate <- mean(bootstrap_results$t)
bootstrap_se <- sd(bootstrap_results$t)

cat("Bootstrap Estimate of sex coefficient:", bootstrap_estimate, "\n")
